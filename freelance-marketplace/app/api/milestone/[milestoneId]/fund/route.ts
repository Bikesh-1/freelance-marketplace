...
export async function POST(req: NextRequest, { params, }: { params: Promise<{ milestoneId: string }>; }) {
  try {
    ...
    if (!milestone) {
      return NextResponse.json({ message: "Milestone not found", },
        { status: 404 }
      );
    }

    if (milestone.job.clientId !== clientProfile.id) {
      return NextResponse.json(
    ...
    if (!milestone.job.selectedFreelancerId) {
      return NextResponse.json(
        { message: "Freelancer is not selected for this job", },
        { status: 400 }
      );
    }
    ...
    if (milestone.status !== "PENDING") {
      return NextResponse.json(
        { message: "Only pending milestones can be funded", }, { status: 400 }
      );
    }
    ...
    const { receipt, transaction, } = await verifyTransaction(transactionHash);
    ...
    const escrowIndex = Number(blockchainEscrowId);

    if (!Number.isInteger(escrowIndex) || escrowIndex < 0) {
      return NextResponse.json(
        { message: "Invalid blockchain escrow ID", },
        { status: 400 }
      );
    }
    ...
    if (Math.abs(fundedAmount - milestone.amount) > 0.000001) {
      return NextResponse.json(
        { message: "Funding amount does not match milestone amount", },
        { status: 400 }
      );
    }
      ...
      async (tx) => {
        const escrow = await tx.escrow.create({
          data: {
            ...
            blockchainEscrowId: escrowIndex,
            network: typeof network === "string" && network.trim() ? network.trim() : "hardhat-local",
            status: "FUNDED",
          },
        });
